float4 GetSkyColor(float3 Dir, float3 LightDir)
{
    ...
    float3 SkyDome = 
        float3(0.1, 0.2, 0.5) *  // Warmer blue (towards dusk)
        lerp(max(ScatteringScale, 0.1), 1.0, saturate(Dir.y)) / max(Dir.y, 0.01);

    SkyDome *= 13.0 / max(length(SkyDome), 13.0);

    float3 Horizon = pow(SkyDome, float3(1.0, 0.8, 0.6));  // A bit warmer with red/pink hues

    SkyColor += lerp(Horizon, SkyDome / (SkyDome + 0.5), saturate(Dir.y * 2.0));

    SkyColor *= 1.0 + pow(1.0 - ScatteringScale, 10.0) * 15.0;  
    SkyColor *= 1.0 - abs(1.0 - Dir.y) * 0.5;
    
    SkyColor = lerp(SkyColor, float3(1.0, 0.6, 0.3), saturate(1.0 - Dir.y));  
